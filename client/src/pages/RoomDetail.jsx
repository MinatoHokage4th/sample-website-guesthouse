...
export default function RoomDetail() {
  ...
  useEffect(() => {
    ...
    if (result) {
      console.log('Room data found:', result.room.type, 'from', result.building.name);
      setData(result);
    } else {
  ...
  const formatPrice = (price, durasi) => {
    const multipliers = { bulan: 1, '3bulan': 3, '6bulan': 6, tahun: 12 };
  ...
  const handleAjukanSewa = (e) => {
    e.preventDefault();
    console.log('Pengajuan sewa:', sewaData);
    alert('Demo: Pengajuan sewa berhasil dikirim! (Simulasi)');
  };
  ...
  if (!data) {
    return (
      <div className="detail-not-found" data-aos="fade-up">
        ...
        <Link to="/rooms" className="btn-primary">Kembali ke Daftar Kamar</Link>
      </div>
    );
  }
  ...
  return (
    <div className="room-detail-page">
      ...
      <div className="breadcrumb" data-aos="fade-right">
        ...
        <span>/</span>
        <span className="current">{room.type} - {building.name}</span>
      </div>

      {/* Judul & Info Singkat */}
      <div className="detail-header" data-aos="fade-up">
        <h2>{room.type} - {building.name}</h2>
      ...
      <div className="detail-layout">
        ...
        <div className="detail-main">
          ...
          {/* Section 1: Foto */}
          <section id="section-foto" className="detail-section" data-aos="fade-up">
            <h3>Foto Kamar</h3>
            {/* Tab Kategori Foto */}
            ...
            <div className="photo-grid">
              {currentImages.map((img, idx) => (
                <div key={idx} className="photo-thumb" onClick={() => openLightbox(idx)}>
          ...
          {/* Section 2: Fasilitas */}
          <section id="section-fasilitas" className="detail-section" data-aos="fade-up">
          ...
          {/* Section 3: Deskripsi */}
          <section id="section-deskripsi" className="detail-section" data-aos="fade-up">
            <h3>Deskripsi Kamar</h3>
            <p>{room.description}</p>
          </section>

          {/* Section 4: Spesifikasi */}
          <section id="section-spesifikasi" className="detail-section" data-aos="fade-up">
            <h3>Spesifikasi</h3>
            <table className="specs-table">
              <tbody>
                <tr><td>Listrik</td><td>{room.specs.listrik}</td></tr>
                <tr><td>Sumber Air</td><td>{room.specs.sumberAir}</td></tr>
                <tr><td>WiFi</td><td>{room.specs.wifi}</td></tr>
                <tr><td>Parkir</td><td>{room.specs.parkir}</td></tr>
                <tr><td>Keamanan</td><td>{room.specs.keamanan}</td></tr>
                <tr><td>Laundry</td><td>{room.specs.laundry}</td></tr>
              </tbody>
            </table>
          </section>

          {/* Section 5: Lokasi */}
          <section id="section-lokasi" className="detail-section" data-aos="fade-up">
            <h3>Lokasi</h3>
        ...
        <aside className="detail-sidebar" data-aos="fade-left">
          <div className="sidebar-card">
            ...
            <div className="sidebar-price">
              <span className="price-current">{room.priceFormatted}</span>
            </div>
            ...
            <form className="sewa-form" onSubmit={handleAjukanSewa}>
              ...
              <select name="durasi" value={sewaData.durasi} onChange={handleSewaChange}>
            ...
            <a
              href={`https://wa.me/62812XXXX?text=Halo, saya tertarik sewa kamar ${room.type} di ${building.name}`}
            ...
            <div className="sidebar-note">
              <p>ℹ️ Bayar DP 30% untuk booking</p>
              <p>📋 Wajib KTP saat check-in</p>
              <p>🕐 Bisa mulai di hari H</p>
            </div>
          </div>
        </aside>
      </div>
      ...
      {lightboxOpen && (
        <div className="lightbox-overlay" onClick={closeLightbox}>
          ...
          <img
            ...
            onError={(e) => { e.target.style.display = 'none'; console.error('Image failed to load:', currentImages[lightboxIndex]); }}
          />
          <button className="lightbox-nav next" onClick={(e) => { e.stopPropagation(); nextImage(); }}>&#10095;</button>
          <div className="lightbox-counter">{lightboxIndex + 1} / {currentImages.length}</div>
        </div>
      )}
    </div>
  );
